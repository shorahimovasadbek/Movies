...
export default function MoviesP() {
  ...
  useEffect(() => {
    async function getData() {
      let baza = await fetch(`${process.env.REACT_APP_START_KEY}/3/movie/upcoming?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&page=1`)
        .then(ress => {
          return ress.json();
        })
        .catch(err => {
  ...
  return (
    <div className='row g-0'>
      <div className='col-3'>
        <h2 className='text-center tv_show1 border shadow fw-bold mt-5'>Upcoming</h2>
