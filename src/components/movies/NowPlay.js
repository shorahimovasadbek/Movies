...
export default function MoviesP() {
  ...
  useEffect(() => {
    async function getData() {
      let baza = await fetch(`${process.env.REACT_APP_START_KEY}/3/movie/now_playing?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&page=1`)
        .then(ress => {
          return ress.json();
        })
        .catch(err => {
  ...
  return (
    <div className='row g-0'>
      <div className='col-3'>
        <h2 className='text-center tv_show1 border shadow fw-bold mt-5'>Now Playing</h2>
            ...
            (mass2.length > 0 && all) && mass2.map((item, i) => {
              return (
                <div className='col-3 my-3' key={i}>
                  ...
                  <h6 className=' text-start mt-4 w-75'>{item.original_title}</h6>
                  <p className='text-start'><span>Dat: </span>{item.release_date}</p>
                </div>
              )
            })
          }
