...
export default function Search_One() {
  ...
  useEffect(() => {
    async function getData() {
      let baza = await fetch(`${process.env.REACT_APP_START_KEY}/3/movie/${id}?api_key=${process.env.REACT_APP_API_KEY}`)
        .then(ress => {
          return ress.json()
        })
        .catch(err => {
  ...
  return (
        <div>
          <ul className='ul_one my-2'>
            <Link to='/'><button className='btn btn-outline-dark fs-5 fw-bold'><BiArrowBack /> Back Home</button></Link>
            <li>Overview</li>
            <li>Media</li>
            <li>Fandom</li>
            <li>Share</li>
          ...
          <div className='One_img_background' style={{ backgroundImage: `url(${`${process.env.REACT_APP_IMG_KEY}` + baza.backdrop_path})`, backgroundPosition: 'center', backgroundSize: 'cover', backgroundRepeat: 'no-repeat', height: "500px", width: "100%" }}>
            <div className='centered_back d-flex justify-content-start px-5 align-items-center'>
              ...
              <div className='px-5'>
                <h2>{baza.original_title} <span>({baza.release_date})</span></h2>
